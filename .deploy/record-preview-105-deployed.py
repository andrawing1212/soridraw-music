...
path = Path('DOCS/CURRENT_RELEASE_STATE.md')
text = path.read_text(encoding='utf-8')
marker = '## 0B. PREVIEW 105 — 1분 이벤트 좋아요 PREVIEW 배포 완료\n'
if marker in text:
    raise SystemExit('PREVIEW 105 deployment state already recorded')

intro = '> 새 채팅은 이 문서 + 실제 GitHub/Firebase/Cloudflare 상태를 기준으로 이어간다. 문서와 실제 상태가 다르면 실제 상태 우선.\n\n'
block = '''## 0B. PREVIEW 105 — 1분 이벤트 좋아요 PREVIEW 배포 완료
...
if intro not in text:
    raise SystemExit('CURRENT_RELEASE_STATE intro anchor missing')
path.write_text(text.replace(intro, intro + block, 1), encoding='utf-8')

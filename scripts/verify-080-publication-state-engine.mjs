...
// Model the already-protected shared revision write. 080 must not remove it because
// older TEST/PRODUCTION code still shares the same canonical D1.
...
assert.equal(db.totalChanges - before, 2, 'private hot transition = canonical row + protected shared revision only');
...
assert.equal(db.totalChanges - before, 2, 'republish/options hot transition = canonical row + protected shared revision only');
...
assert.ok(db.totalChanges - before > 2, 'real canonical content edit still refreshes derived recovery snapshot');
...
assert.match(favorites, /state\.registered[\s\S]{0,500}setExploreTrackVisibility\(user,\s*state\.trackId,\s*true,\s*options\)/,
...
if (process.env.SORIDRAW_GENERATED_WORKER) {
  ...
  assert.ok(visibility.includes('LEFT JOIN track_stats'), 'visibility route needs bounded track stats lookup');
}

console.log('PASS 080: registered republish uses one visibility route; private/republish logical D1 writes are 2 including protected shared revision; indexed publish time stays stable; cold repair validates canonical visibility.');

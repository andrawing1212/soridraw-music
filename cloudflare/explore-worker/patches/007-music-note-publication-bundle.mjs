...
const bootstrapRoute = includeBootstrap
  ? `if (url.pathname === "/v1/internal/bootstrap-music-note-publication-bundles" && request.method === "POST") {\n      return await bootstrapMusicNotePublicationBundles(request, env, cors);\n    }\n    `
  : '';
const route = `${bootstrapRoute}if (url.pathname === "/v1/me/music-note-publications-bundle" && request.method === "GET") {\n      return await handleMusicNotePublicationBundle(request, env, cors);\n    }\n    `;
...
if (includeBootstrap && !src.includes('/v1/internal/bootstrap-music-note-publication-bundles')) throw new Error('[007] bootstrap route missing after patch');
if (!includeBootstrap && src.includes('/v1/internal/bootstrap-music-note-publication-bundles')) throw new Error('[007] bootstrap route leaked into final Worker');
if ((src.match(new RegExp(MARKER, 'g')) || []).length !== 1) throw new Error('[007] marker count mismatch');

fs.writeFileSync(path, src, 'utf8');
console.log(`[007] publication bundle applied mode=${includeBootstrap ? 'bootstrap' : 'final'}`);

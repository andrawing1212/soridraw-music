...
if MARKER not in app:
    ...
    app = replace_once(
        ...
        "          markCacheDiagnostic('musicNote', snapshot.metadata.fromCache ? 'CACHE' : 'SYNC', snapshot.metadata.fromCache ? 0 : Math.max(1, snapshot.docs.length));",
...
if MARKER not in library:
    library = replace_once(
        ...
        "        markCacheDiagnostic('library', snapshot.metadata.fromCache ? 'CACHE' : 'SYNC', snapshot.metadata.fromCache ? 0 : Math.max(1, docs.length));",
...
print('Applied SORIDRAW 897 accuracy: Firestore cache snapshots stay CACHE 0; server snapshots use billable-read floor 1.')

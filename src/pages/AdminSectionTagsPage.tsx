...
function normalizeTagTier(tier: any): TagTier {
  if (tier === 'free') return 'free';
  if (tier === 'basic') return 'basic';
  if (tier === 'pro') return 'pro';

  // legacy 값 처리
  if (tier === 'pro+' || tier === 'proPlus') return 'pro';
...
export default function AdminSectionTagsPage({ isAdmin: isAdminProp }: { isAdmin?: boolean }) {
  ...
  useEffect(() => {
    ...
    window.history.pushState({ modal: 'tag' }, '');
    ...
    return () => {
      ...
      // If closing manually (save/cancel), remove the pushed state
      if (modalHistoryRef.current) {
        window.history.back();
        modalHistoryRef.current = false;
      }
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        const fetchedTags = snapshot.docs.map((snapshotDoc) => {
          ...
          const normalizedTier = normalizeTagTier(data.tier);
          return {
            ...data,
  ...
  const handleEdit = (tag: SectionTagDoc) => {
    ...
    setFormData({
      ...
      tier: normalizeTagTier(tag.tier),
    ...
    try {
      ...
      if (isEditing === 'new') {
        ...
        const finalData: SectionTag = {
          ...
          tier: normalizeTagTier(formData.tier),
      ...
      for (const label of allLabels) {
        ...
        const tagData: SectionTag = {
          ...
          description:
            ...
            `${label}에 대한 설명입니다.`,
          tier: normalizeTagTier(meta?.tier),
          sections,
          isActive: true,
          createdAt: Date.now(),
          updatedAt: Date.now()
  ...
  return (
    ...
    >
      ...
      {isEditing && (
        ...
        >
          <div 
            className="w-full max-w-2xl p-6 bg-[var(--card-bg)] rounded-2xl border border-[var(--border-color)] shadow-2xl animate-in zoom-in-95"

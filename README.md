...
## 820차 — 라이트 뮤직노트/라이브러리 디테일창 색상 소유권 분리
- 기준: 819차.
- 원인 확인: `index.css`의 공용 라이트 팝업 변환 규칙과 뒤에서 로드되는 `studioLayout.css`의 Music Note/Library 디테일 공통 강제 규칙이 동시에 `!important`로 같은 창/버튼을 덮고 있어 최근 라이트 톤 수정이 디테일창에 제대로 드러나지 않았다.
- Music Note/Library 디테일 패널을 공용 라이트 팝업 변환 대상에서 제외하고, `studioLayout.css` 마지막 계층에 두 디테일창 전용 팔레트 소유권을 배치했다.
- 외부 쉘 → 헤더/푸터 → 본문 → 카드 → 내부 정보칸 → 중립 버튼 순으로 단계가 보이도록 웜그레이 명도를 분리했다. 버튼/창 외곽선은 추가하지 않았다.
- Music Note는 코랄, Library는 바이올렛의 기존 정체성 강조색을 라이트모드에 맞는 명도로 복구했다.
- Music Note에 항상 켜져 있던 다크용 색상 wash 오버레이는 Classic Light에서만 끄고 Dark/Studio Black에는 유지했다.
- 레이아웃/크기/기능/Firebase/Auth/Firestore/Functions는 변경하지 않았다.

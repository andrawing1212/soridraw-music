...
if 'SORIDRAW_TOOLTIP_ISOLATED_HOST_984' in s:
    print('tooltip isolated host already applied; no source patch needed')
    raise SystemExit(0)

portal = """function Portal({ children, enabled = true }: { children: React.ReactNode; enabled?: boolean }) {
  if (typeof document === 'undefined') return null;
  return enabled ? createPortal(children, document.body) : <>{children}</>;
}
"""
if s.count(portal) != 1:
    raise SystemExit(f'Portal anchor count mismatch: {s.count(portal)}')
...
"""
s = s.replace(portal, host, 1)

old_state = """  type StudioDescriptionPane = 'builder' | 'result' | 'global';
  ...
  const [isTooltipHovered, setIsTooltipHovered] = useState(false);
"""
new_state = """  type StudioDescriptionPlacement = StudioDescriptionOverlayPlacement;
...
if s.count(old_state) != 1:
    raise SystemExit(f'tooltip state block count mismatch: {s.count(old_state)}')
s = s.replace(old_state, new_state, 1)

old_setter = """  const setHoveredItem = useCallback((item: CategoryItem | null) => {
...
"""
new_setter = """  const commitHoveredItem = useCallback((item: CategoryItem | null) => {
...
if s.count(old_setter) != 1:
    raise SystemExit(f'tooltip setter block count mismatch: {s.count(old_setter)}')
...
"""
if s.count(old_effects) != 1:
    raise SystemExit(f'tooltip effect block count mismatch: {s.count(old_effects)}')
s = s.replace(old_effects, '', 1)

old_owner_clear = """  useEffect(() => {
    if (studioActionOwner === 'floating') return;
    setHoveredItemState((current) => (isActionButtonHintItem(current) ? null : current));
  }, [studioActionOwner]);
"""
new_owner_clear = """  useEffect(() => {
    if (studioActionOwner === 'floating') return;
...
"""
if s.count(old_owner_clear) != 1:
    raise SystemExit(f'action owner clear block count mismatch: {s.count(old_owner_clear)}')
s = s.replace(old_owner_clear, new_owner_clear, 1)

external_hover_clear = '    setIsTooltipHovered(false);\n'
...
old_clear_action = """  const clearActionButtonHint = () => {
    setHoveredItemState((current) => (isActionButtonHintItem(current) ? null : current));
  };
"""
new_clear_action = """  const clearActionButtonHint = () => {
    const current = studioDescriptionCurrentItemRef.current;
    if (isActionButtonHintItem(current)) setHoveredItem(null);
  };
"""
if s.count(old_clear_action) != 1:
    raise SystemExit(f'clearActionButtonHint count mismatch: {s.count(old_clear_action)}')
s = s.replace(old_clear_action, new_clear_action, 1)

tooltip_start = s.index('      {/* Tooltip / Description Overlay */}')
next_presence = s.index('\n\n      <AnimatePresence>', tooltip_start + 20)
new_overlay = """      {/* Tooltip / Description Overlay — isolated persistent host */}
      ...
      />"""
s = s[:tooltip_start] + new_overlay + s[next_presence:]

hovered_prop = '              hoveredItem={hoveredItem}\n'
if s.count(hovered_prop) != 2:
    raise SystemExit(f'CategorySection hoveredItem callsite count mismatch: {s.count(hovered_prop)}')
s = s.replace(hovered_prop, '')

prop_line = '  hoveredItem: CategoryItem | null;\n'
if s.count(prop_line) != 1:
    raise SystemExit(f'CategorySection hoveredItem prop count mismatch: {s.count(prop_line)}')
s = s.replace(prop_line, '', 1)

destructure_line = '  hoveredItem,\n'
if s.count(destructure_line) != 1:
    raise SystemExit(f'CategorySection hoveredItem destructure count mismatch: {s.count(destructure_line)}')
s = s.replace(destructure_line, '', 1)

css_start_marker = '  /* 983 — CPU-only idle pointer budget.'
css_end_marker = '  /* 576 — browser-native leaf isolation.'
if css.count(css_start_marker) != 1 or css.count(css_end_marker) != 1:
    raise SystemExit(f'CSS marker mismatch: 983={css.count(css_start_marker)} 576={css.count(css_end_marker)}')
css_start = css.index(css_start_marker)
css_end = css.index(css_end_marker, css_start)
css = css[:css_start] + css[css_end:]

app_path.write_text(s, encoding='utf-8')
css_path.write_text(css, encoding='utf-8')
print('tooltip isolation source patch applied')

...
if (source.includes('finalizeMusicNotePublic025') && source.includes('handleVisibilityR2CoreLegacy025')) {
  console.log('[025] publication UPDATE RETURNING hot path already applied.');
  process.exit(0);
}

// Validate durable runtime shape, not comments: Wrangler may remove patch markers.
...
async function handleMusicNotePublicationSingleWrite025(request, env, cors, authContext, source, publicationOptions) {
  ...
  // Normal private -> public is a pure visibility mutation. Match the canonical
  // payload in the same primary-key statement so no pre-SELECT is required.
  // Any content/option mismatch falls back before visibility is changed.
  ...
  if (!transitioned) {
    return await handleMusicNotePublicationSingleWrite016Legacy025(
      request, env, cors, authContext, source, publicationOptions
    );
  }

  return await finalizeMusicNotePublic025(
    request, env, cors, authContext, source, publicationOptions, transitioned, now
  );
}
...
async function handleVisibilityR2Core025(request, env, cors, trackId) {
  ...
  // No-op private, non-Music-Note, public=true and not-found retain old behavior.
...
if (!currentPublic.includes('publicationReadState016(')) throw new Error('[025] expected 024 public pre-read shape missing');
...
if (!currentVisibility.includes('FROM tracks')) throw new Error('[025] expected 024 private pre-read shape missing');
...
for (const required of ['UPDATE tracks', 'SET is_public = 1, updated_at = ?', 'RETURNING *', 'handleMusicNotePublicationSingleWrite016Legacy025(', 'finalizeMusicNotePublic025(']) {
  if (!publicHot.includes(required)) throw new Error(`[025] public hot-path invariant missing: ${required}`);
}
for (const required of ['UPDATE tracks', 'SET is_public = 0, updated_at = ?', 'RETURNING id, owner_uid', 'handleVisibilityR2CoreLegacy025(', 'finalizeMusicNotePrivate025(']) {

...
## 578차 — Lite V2 외부 UI live write 최소화
- 기준: 577차. 뮤직노트+라이브러리 공통 성능 테스트 2단계이며 디자인/실시간 pane 리플로우/leaf isolation은 그대로 유지한다.
- Studio hero 검색 버튼의 `right/left/transform` 직접 갱신을 제거하고, 검색 버튼과 masthead host의 최소 공통 부모인 `.soridraw-studio-shell`에 `--soridraw-studio-builder-width` 1개만 발행해 기존 CSS가 위치를 계산하도록 변경했다.
- Studio Black 데스크톱에서 이미 `display:none`인 body live-keyword portal의 left/right 매프레임 갱신을 제거했다.
- expanded floating action bar에서 실제 CSS가 사용하지 않는 `--soridraw-studio-builder-width` 중복 write를 제거했다.
- 좌우 접기 버튼, hero width, action bar left/width, collapsed action width처럼 실제 기하를 소유하는 값은 마지막 inline 값과 동일하면 setProperty 자체를 생략한다. left rail edge처럼 드래그 중 변하지 않는 값은 최초 1회만 기록된다.
- root 전역 split CSS 변수는 기존처럼 드래그 중 갱신하지 않으며 시작/종료/외부 레이아웃 커밋에서만 사용한다.
- Firebase/Auth/Firestore/Functions/저장 구조 변경 없음.

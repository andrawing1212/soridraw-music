...
const sharedHelpers = source.slice(source.indexOf(`// ${marker}`), functionRange('handlePublicProfileFirstViewWithEdgeCacheCore060').start);
if (sharedHelpers.includes('env.DB.') || sharedHelpers.includes('.prepare(')) {
  throw new Error('[060] shared profile parity helpers must not read D1');
}

writeFileSync(workerPath, source, 'utf8');
console.log('[060] Public-profile reads now prefer a shared R2 mirror; actual profile/like/follow/publication mutations prime local R2 from shared state and mirror the targeted profile back.');

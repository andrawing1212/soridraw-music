...
${tagOrderInstruction}
- Section tags contain only singer ownership/performance direction and the section's local function. In multi-vocal songs the fixed order is exact short singer anchor first, then one short performance cue. In solo songs use only the local performance cue. Put real instrument, sound-effect, ambience, foley, reverb, and texture-event directions on a separate square-bracket line directly below the section tag.

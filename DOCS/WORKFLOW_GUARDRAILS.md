...
### 사용자 수정 지시의 PREVIEW 자동 배포 규칙 — 2026-09-22
- 사용자가 "수정해", "고쳐", "진행해"처럼 실제 수정 작업을 지시했고, 수정 결과가 사전검사 PASS이며 PREVIEW에 안전하게 배포 가능한 상태라면 **별도 배포 승인 질문 없이 같은 작업에서 PREVIEW 배포까지 진행**한다.
- 사용자가 "배포하지 마", "소스만 수정"처럼 명확히 제한한 경우에만 PREVIEW 배포를 생략한다.
- 이 자동 규칙은 **PREVIEW에만 적용**한다.
- TEST 승격은 기존 TEST 승격 기준을 따르고, PRODUCTION은 사용자의 명확한 정식배포 승인 없이는 절대 진행하지 않는다.
- D1 migration/seed/대량변경/파괴적 변경처럼 별도 승인이 필요한 작업은 이 자동 배포 규칙의 예외다.

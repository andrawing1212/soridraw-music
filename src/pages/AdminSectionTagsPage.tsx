...
import { TAG_META, ALLOWED_TAGS_BY_SECTION, TAG_DESCRIPTIONS, TagTier } from '../constants';
...
export default function AdminSectionTagsPage() {
  ...
  const handleEdit = (tag: SectionTag) => {
    setIsEditing(tag.id);
    setFormData(tag);
    setError(null);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing === 'new') {
        ...
        const existingDoc = await getDoc(doc(db, 'section_tags', id));
        ...
        await setDoc(doc(db, 'section_tags', id), finalData);
      } else if (isEditing) {
        const finalData = {
          ...formData,
          updatedAt: Date.now()
        };
        const tagRef = doc(db, 'section_tags', isEditing);
        await updateDoc(tagRef, finalData);
      }
      setIsEditing(null);
    } catch (err) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteDoc(doc(db, 'section_tags', id));
    } catch (err) {
  ...
  const handleToggleActive = async (tag: SectionTag) => {
    try {
      await updateDoc(doc(db, 'section_tags', tag.id), {
  ...
  const handleInitialLoad = async () => {
    if (!window.confirm("constants.ts의 데이터를 Firestore로 마이그레이션하시겠습니까? (이미 존재하는 태그는 건너뜁니다)")) return;
    
    setIsMigrating(true);
    setError(null);
    
    try {
      // 1. Get existing tags to avoid duplicates by ID
      ...
      // 2. Iterate through all tags in TAG_META
      for (const [label, meta] of Object.entries(TAG_META)) {
        const id = label.toLowerCase().replace(/\s+/g, '-');
        
        // Skip if already exists
        if (existingIds.has(id)) continue;

        // 3. Find which sections this tag belongs to
        ...
        // 4. Create document reference with the derived ID
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[var(--bg-color)]">
        <div className="text-center p-8 bg-[var(--card-bg)] rounded-2xl border border-[var(--border-color)] shadow-xl">
          ...
          <button onClick={() => navigate('/')} className="mt-6 px-6 py-2 bg-brand-orange text-white rounded-xl font-bold">홈으로</button>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <button onClick={() => navigate('/')} className="p-2 hover:bg-white/5 rounded-lg text-[var(--text-secondary)]">
              ...
              <tbody className="divide-y divide-[var(--border-color)]">
                {isLoading ? (
                  ...
                  filteredTags.map((tag) => (
                    <tr key={tag.id} className="hover:bg-[var(--hover-bg)]/30 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={cn(
                          ...
                          tag.tier === 'free' ? "bg-gray-500/10 text-gray-400" :
                          tag.tier === 'pro' ? "bg-brand-orange/10 text-brand-orange" :
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-wrap gap-1 max-w-xs">
                          {tag.sections.map(s => (
                            <span key={s} className="px-1.5 py-0.5 bg-white/5 border border-white/10 rounded text-[9px] text-[var(--text-secondary)]">
                              {s}
                            </span>
                          ))}
                        </div>

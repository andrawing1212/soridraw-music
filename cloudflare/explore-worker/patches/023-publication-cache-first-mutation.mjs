...
for (const required of [
  ...
  'handlePublicationOptionsR2Core',
...
async function publicationResolveProfileHandle023(env, uid, preferredHandle) {
  ...
  try {
    const row = await env.DB.prepare('SELECT handle FROM public_profiles WHERE uid = ? LIMIT 1').bind(uid).first();
...
async function patchExploreProfileR2Mutation023(env, uid, change) {
  try {
    ...
    // Track mutations already know the canonical UID. Avoid the handle-alias lookup
    // and alias rewrite here; aliases change only when profile identity changes.
    const bundle = await readExploreR2Json(env, exploreProfileR2Key(normalizedUid));
    if (!validExploreProfileR2Bundle020(bundle)) {
      console.warn('[SORIDRAW 023] canonical profile R2 bundle missing; bounded repair deferred to profile access.');
    ...
    if (!change?.remove) {
      ...
      const nextItem = change?.item ? { ...previous, ...change.item } : { ...previous, ...(change?.patch || {}) };
      items.push(nextItem);
    }
    ...
    const nextBundle = { ...bundle, revision: nextRevision, updatedAt: Date.now(), body: { ...bundle.body, data: nextData } };
...
async function handleMusicNotePublicationOptions023(request, env, cors, authContext, body, row) {
  ...
  `).bind(next.allowNextSongApply, next.allowFollowerSave, next.profilePinned, now, row.id, authContext.uid).run();
...
async function handlePublicationOptionsR2Core023(request, env, cors, trackId) {
  ...
  if (String(row.source_type || '') !== 'music_note') return await handlePublicationOptionsR2CoreLegacy017(request, env, cors, trackId);
...
async function handleVisibilityR2Core023(request, env, cors, trackId) {
  const probe = request.clone();
  const authContext = await requireExploreAuth(probe);
  const body = await readJsonBody(probe, 2048);
...
replaceFunction('applyPublicationVisibilityTransition021', applyPublicationVisibilityTransition023.toString().replace('applyPublicationVisibilityTransition023', 'applyPublicationVisibilityTransition021'));
replaceFunction('patchExploreProfileR2Mutation019', patchExploreProfileR2Mutation023.toString().replace('patchExploreProfileR2Mutation023', 'patchExploreProfileR2Mutation019'));
replaceFunction('handleMusicNotePublicationOptions017', handleMusicNotePublicationOptions023.toString().replace('handleMusicNotePublicationOptions023', 'handleMusicNotePublicationOptions017'));
replaceFunction('handlePublicationOptionsR2Core', handlePublicationOptionsR2Core023.toString().replace('handlePublicationOptionsR2Core023', 'handlePublicationOptionsR2Core'));
replaceFunction('handleMusicNotePrivate017', handleMusicNotePrivate023.toString().replace('handleMusicNotePrivate023', 'handleMusicNotePrivate017'));
replaceFunction('handleVisibilityR2Core', handleVisibilityR2Core023.toString().replace('handleVisibilityR2Core023', 'handleVisibilityR2Core'));
...
if (!transition.includes('AND is_public <> 1') || !transition.includes('AND is_public <> 0')) throw new Error('[023] DB idempotency guard missing');
const profilePatch = functionRange('patchExploreProfileR2Mutation019').text;
if (!profilePatch.includes('readExploreR2Json(env, exploreProfileR2Key(normalizedUid))')) throw new Error('[023] canonical UID R2 read missing');
if (profilePatch.includes('readExploreProfileCanonicalR2Bundle020')) throw new Error('[023] alias-first profile read still active');
if (profilePatch.includes('writeExploreProfileAlias020')) throw new Error('[023] unchanged handle alias still rewritten');
const privateHandler = functionRange('handleMusicNotePrivate017').text;
if (!privateHandler.includes("mutation: 'idempotent'") || !privateHandler.includes('Promise.allSettled')) throw new Error('[023] private idempotent/parallel invariant failed');
const optionHandler = functionRange('handleMusicNotePublicationOptions017').text;
if (!optionHandler.includes("mutation: 'idempotent'") || !optionHandler.includes('Promise.allSettled')) throw new Error('[023] option idempotent/parallel invariant failed');
for (const name of ['handleVisibilityR2Core', 'handlePublicationOptionsR2Core']) {
  const fn = functionRange(name).text;
  if (fn.includes('LEFT JOIN public_profiles') || fn.includes('profile_handle')) throw new Error(`[023] ${name} still reads public profile on hot mutation path`);
}
...
console.log('[023] Public/private mutations preserve publication chronology, skip profile joins/alias churn, and short-circuit no-op cache work.');

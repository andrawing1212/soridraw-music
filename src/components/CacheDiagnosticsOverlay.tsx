...
export default function CacheDiagnosticsOverlay({ isAdmin }: { isAdmin: boolean }) {
  ...
  const totals = useMemo(() => rows.reduce((acc, row) => {
    ...
    acc.server += state.reads;
    acc.cache += state.cacheHits;
    return acc;
  }, { server: 0, cache: 0 }), [states]);
    ...
    <div className="fixed right-3 top-[76px] z-[9998] w-[244px] rounded-2xl bg-black/80 px-3 py-2.5 text-white/85 shadow-2xl backdrop-blur-md">
      <div className="mb-2 flex items-center justify-between gap-2">
        <div className="min-w-0">
          ...
          <div className="mt-0.5 text-[9px] font-bold text-white/42">서버 {totals.server} · 캐시 {totals.cache}</div>
        </div>
        <button
          type="button"
          onClick={() => {
      ...
      <div className="space-y-1">
        {rows.map(({ domain, label }) => {
          ...
          return (
            <div key={domain} className="grid grid-cols-[58px_42px_1fr] items-center gap-1 text-[9px] font-bold leading-5">
              ...
              <span className="text-right tabular-nums text-white/58">서버 {state.reads} · 캐시 {state.cacheHits}</span>
            </div>
          );
        })}
      </div>

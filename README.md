...
## 591차 — 직접 pane 좌표를 실제 Lite V2 런타임으로 승격

- 기준: 590차 고정 1400×900 좌표 A/B 진단.
- 590 자동 A/B에서 CSS 변수 좌표 대비 직접 pane 좌표가 DEV/PROD 모두 큰 폭으로 개선되어, 진단 전용 direct 경로를 실제 Lite V2 기본 드래그 경로로 승격했다.
- 드래그 매 프레임에는 부모의 상속 CSS custom property를 갱신하지 않고 builder/result/divider 세 geometry owner에만 직접 pixel 좌표를 쓴다.
- 최종 분할 비율 저장, collapse 상태, 반응형 mode 전환, 외부 검색/액션바 위치 동기화, root 최종 측정값 commit은 기존 계약을 유지한다.
- 관리자 좌표 A/B는 유지한다. 일반 자동 테스트의 기본 모드는 이제 실제 런타임과 동일한 `direct`이며, `CSS 변수 좌표` 프로필을 명시적으로 실행할 때만 legacy 경로로 전환한다.
- 590의 1400×900 고정 표면/스크롤/이동거리 자동화와 DEV/PROD 진단서는 그대로 유지한다.

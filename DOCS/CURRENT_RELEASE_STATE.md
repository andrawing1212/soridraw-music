...
**추가 사용자 실사용 3곡**:
1. 30.0초 / 호출 2회 / 총 29,595 token.
   - `gemini-3.6-flash`: 기존 cooldown 4분18초 남아 건너뜀.
   - `gemini-3.5-flash`: 0.703초, HTTP 429 실패.
   - `gemini-3.5-flash-lite`: 24.8초, 성공.
2. 1분11초 / 호출 2회 / 총 28,831 token.
   - `gemini-3.6-flash`: 36.1초, stream 오류.
   - `gemini-3.5-flash-lite`: 29.0초, 성공.
3. 1분2초 / 호출 1회 / 총 30,019 token.
   - `gemini-3.6-flash`: 다른 생성이 같은 모델 시험 중이라 in-flight 보호로 건너뜀.
   - `gemini-3.5-flash`: 57.7초, 성공.

**app182 총 4곡 실사용 합계 (직전 0HC 포함)**:
- 성공 4/4.
- 물리 Gemini 호출 총 7회 = 곡당 평균 **1.75회**.
- 총 처리시간 209.5초 = 곡당 평균 **52.4초**.
- 총 token 116,423 = 곡당 평균 **29,106 token**.
- `gemini-3.6-flash` 실제 호출 2회는 둘 다 stream 오류; 나머지 2곡은 cooldown/in-flight 보호로 선제 생략.
- `gemini-3.5-flash`: 성공 2회(32.9s, 57.7s), 429 실패 1회(0.703s).
- `gemini-3.5-flash-lite`: 실제 호출 2회 모두 성공(24.8s, 29.0s).
- 병렬 생성 시 같은 모델을 중복 시험하지 않는 in-flight 보호가 실사용에서 정상 작동 확인.

**판정**:
- app182는 기존 5회/131초 패턴 대비 호출 수와 평균 지연을 크게 낮춘 상태로 **유지 가치가 있음**.
- 현재 병목은 후처리 재호출보다 최초 모델 가용성/응답 지연임.
- 특히 3.6은 이번 4곡 창에서 실제 호출 2/2 실패이므로 단기 건강도 기반 후순위 후보지만, 표본이 작고 품질 우선순위가 있으므로 영구 모델 순서 변경은 아직 금지.
- 다음 최적화 후보는 서버/DB 추가 없이 브라우저 메모리에서 최근 모델 실패/429/느린 성공을 짧게 기억해 **다음 곡에서만 일시적으로 후순위**로 보내는 방식. 모든 후보가 불안정하면 원래 순서를 복원해 기능 보호.
- 3.5-lite가 이번 표본에서 2/2 성공 및 24.8~29.0초로 가장 안정적/빠르게 관찰됐지만, 가사/작곡 품질 비교 없이 기본 1순위로 영구 승격하지 않음.
- TEST/main 승격 전, PRODUCTION 비변경.


## 0HC. app182 실사용 1곡 검증 — 호출 2회 / 46.5초 / 27,978 token (2026-09-26 KST)

**사용자 실사용 결과**:
- PREVIEW app182 일반 V1 생성 1곡 성공.
- 총 처리시간: **46.5초**.
- 총 물리 Gemini 호출: **2회**.
- 총 token: **27,978** (입력 25,239 / 출력 2,739 / 추론 0).
- 1차 `gemini-3.6-flash`: 5.0초, `Gemini stream reported an error`, 실패.
- 2차 `gemini-3.5-flash`: 32.9초, 성공.
- 별도 금지어/후처리 Gemini 호출 없음.

**app181 직전 실사용과 비교**:
- 직전 기준: 5회 / 2분 11초(131초) / 30,374 token.
- app182: 2회 / 46.5초 / 27,978 token.
- 물리 호출 5 → 2 (**60% 감소**).
- 총 처리시간 131초 → 46.5초 (**84.5초 단축, 약 64.5% 감소**).
- 총 token 30,374 → 27,978 (**2,396 token 감소, 약 7.9% 감소**).
- 단, 이번 곡은 금지어/후처리 자체가 발생하지 않았으므로 app182의 `같은 곡 실패/slow-success 모델 후처리 재호출 방지` 분기는 실제로 발동하지 않음. 해당 로직은 자동감사 PASS 상태이나 실사용 분기 검증은 다음 해당 조건 발생 시 확인.

**현재 판정**:
- app182 1차 목표였던 호출수/생성시간 안정화는 이번 실사용에서 **유의미한 개선 확인**.
- `gemini-3.6-flash`의 stream 오류는 여전히 남아 있으므로 provider/모델 가용성 불안정 자체가 해결된 것은 아님.
- 현 시점에서 최초 모델 순서를 즉시 재배열하지 않고 app182를 유지. 일반 생성 2~3회 추가 관찰 후 3.6이 반복 실패하는 경우에만 짧은 시간 모델 건강도 기반 임시 우선순위 조정을 검토.
- 품질 프롬프트/가사 규칙/5단 구조는 계속 동결.
- TEST/main 승격 전, PRODUCTION 비변경.

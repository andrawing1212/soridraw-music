...
## SORIDRAW 794차 — 764 체감 복구용 생성바 live writer 경량화
- 기준: `SORIDRAW_793차_모바일_태블릿가로_생성바_심플형_PC전용분리.zip`.
- 목적: 764차 Pure Pane 기본 승격 이후 790~793에서 추가된 생성바 실시간 추적 기능은 유지하면서, 분할바 이동 시 생긴 미세한 프레임 끊김을 줄인다.
- 원인 후보를 764 이후 연속 hot-path 차이로 좁혔다. 현재 생산 경로에서 매 rAF마다 새로 추가된 가장 큰 지속 작업은 `syncGenerationBarGeometry`의 floating 생성바 geometry writer였다.
- 790~793은 floating 생성바 요소에 `--soridraw-action-fixed-left` / `--soridraw-action-fixed-width` 커스텀 속성을 매 픽셀 기록했다. 이 값은 상속 가능한 CSS custom property라 생성바 하위 subtree까지 style invalidation 범위를 넓힐 수 있다.
- 수정: floating 생성바의 live geometry를 상속 custom property 대신 해당 portal 요소의 직접 `left / width / translate`로 기록한다. `left`는 드래그 동안 0px에 고정하고 실제 x 이동은 CSS 독립 `translate`로 처리해, 생성바 최대폭이 고정되는 PC 구간에서는 수평 이동이 layout 위치 변경이 아니라 compositor 이동에 가깝게 동작하도록 했다.
- 생성바 폭이 실제로 줄어드는 좁은 Builder 구간에서는 `width`만 직접 갱신한다. 내부 심플형/PC형 디자인 판정은 기존 791/793의 660/1080 경계 실시간 전환을 그대로 유지한다.
- 접힌 생성버튼은 왼쪽 rail에 고정된 요소이므로 horizontal split 동안 Builder 폭 custom property를 매 픽셀 다시 쓸 필요가 없다. drag hot path의 연속 writer에서 제외하고 pointer-up resting geometry가 최종 크기를 다시 소유하도록 했다.
...
### 807차 — 섹션 커스텀 현재 구조 카드 강조 + 분할모드 외곽선 제거
- 기준: 806차.
- 다크/라이트/분할 공통으로 `현재 구조` 작업영역의 배경색은 유지하고, 추가된 섹션 행 카드만 한 톤 밝게 분리했다.
- 섹션 행 카드에 마우스를 올리면 살짝 밝아지고, 클릭하면 배경색만 한 단계 더 강조된다. 같은 카드를 다시 클릭하면 선택 강조가 해제된다.
- 선택 상태는 기존 삽입 위치 선택 기능과 같은 상태를 사용해 별도 저장 구조를 추가하지 않았다.
- 선택 강조에서 border/ring을 사용하지 않고 배경색만 사용한다.
- 806차에서 Classic Dark/Light에만 적용했던 섹션 커스텀 내부 외곽선 제거를 Split Mode에도 동일 적용했다.
- Firebase/Auth/Firestore/Functions/저장 구조 변경 없음.

### 808차 — 라이트모드 전체 톤다운
- Classic Light 전용 중립 팔레트를 한 단계 어둡게 조정했습니다.
- 외부 캔버스, 내부 패널/카드, 입력창, 중립 버튼이 같은 따뜻한 회색 계열로 이어지도록 공통 토큰을 재연결했습니다.
- 흰색으로 남던 일부 Studio 중립 버튼도 라이트 전용 control 톤으로 맞췄습니다.
- 섹션 커스텀 현재 구조 카드의 807차 hover/선택 동작은 유지하면서 새 라이트 톤에 맞게 밝기를 낮췄습니다.
- 브랜드 주황/선택 강조색은 유지했고 Classic Dark 및 Studio Black/분할모드는 변경하지 않았습니다.

## 810차 — 라이트 808 톤 복귀 + 메뉴 카드 그림자만 추가
- 기준: 808차 라이트 전체 톤다운 상태로 되돌림(809차의 추가 색상/팝업 톤 변경은 포함하지 않음).
- Classic Light의 장르~템포/보컬/가사/스토리보드 메뉴 카드와 명령창은 기존 808 배경색을 그대로 유지.
- 카드 대비만 조금 확보하도록 기존 Studio shadow surface의 라이트모드 그림자 토큰만 강화.
- 팝업 내부색, 버튼색, 배경색, 다크모드, 분할모드는 변경하지 않음.

### 811차 — 라이트 메뉴 펼치기/팝업 대비 미세조정
- 기준: 810차(808 라이트 톤 + 카드 그림자)를 그대로 유지했습니다.
- 라이트모드 장르 펼치기 영역의 선택 키워드만 노란색으로 남던 예외를 제거하고 다른 메뉴와 같은 웜그레이 텍스트로 통일했습니다.
- 장르/스타일/사운드/분위기/주제 펼치기 영역을 너무 어둡지 않은 중간 웜그레이로 낮춰 카드 본체와 구분되게 했습니다.
- 장르/스타일/사운드/분위기/주제 및 섹션 커스텀 팝업의 중립 내부 버튼/카드만 한 톤 어둡게 했습니다. 선택된 주황 강조 상태는 그대로 유지합니다.
- 다크모드/분할모드, 기능/데이터/Firebase 구조는 변경하지 않았습니다.

## 812차 — 라이트 팝업/펼치기 버튼 명도 미세복구
- 기준: 811차.
- 811차에서 과하게 어두워진 라이트모드 중립 버튼/팝업 카드를 다시 밝힘.
- 장르~주제 펼치기 요약 영역은 811보다 밝게 조정하되 810보다 아주 살짝 어두운 웜그레이로 유지.
- 장르/스타일/사운드 계열 팝업과 섹션 커스텀 내부의 중립 버튼/카드는 811의 진한 베이지에서 연한 웜그레이로 복구.
- 선택 주황색, 808 전체 배경 톤, 810 카드 그림자, 장르 선택키워드 텍스트 통일은 유지.
- 다크모드/분할모드/Firebase/Auth/Firestore/Functions 변경 없음.

## 813차 — 라이트 보컬 솔로/그룹 비선택 톤 통일
- 기준: 812차.
- 라이트모드 보컬 메뉴의 `솔로 / 그룹` 선택기에서 비선택 탭이 가사 `작사 스타일`의 비선택 탭과 같은 톤으로 보이도록 통일.
- 선택된 탭의 주황색 강조는 그대로 유지.
- 다크모드와 분할모드는 변경하지 않음.


## 814차 — 라이트 보컬 캐릭터 만들기 톤다운

- 기준: 813차.
- 라이트모드 `보컬 캐릭터 만들기` 런처의 거의 흰 배경만 따뜻한 중간 회색(`#DDD6CE`)으로 한 단계 낮췄다.
- hover는 아주 조금만 더 어둡게(`#D6CEC5`) 처리해 과한 대비를 만들지 않는다.
- 버튼 크기/문구/아이콘/레이아웃은 그대로이며, 다크모드와 Studio Black 분할모드는 변경하지 않았다.

## 815차 — 라이트 보컬 캐릭터 명도 미세 상향
- 기준: 814차.
- 라이트모드 `보컬 캐릭터 만들기` 런처 배경을 아주 조금만 더 밝게 올려 과한 톤다운 느낌을 완화.
- hover는 814차 기본톤 수준으로 맞춰 클릭 전후 이질감 없이 유지.
- 다크모드와 Studio Black/분할모드는 변경하지 않음.

## 816차 — 라이트 결과영역 갈색 보조강조 + 생성옵션 팝업 대비 보강
- 기준: 815차.
- 라이트모드 결과영역의 `스타일 프롬프트 / 가사 카드 / Music API 생성 / 하단 생성바` 등에 남아있던 밝은 노란 보조강조 텍스트·아이콘을 버튼색과 덜 겹치도록 갈색계열 보조강조로 이동.
- `생성 옵션 선택` 팝업은 외곽 배경, 내부 패널, 중립 버튼 배경을 한 톤씩 눌러 버튼과 배경 구분이 더 잘 보이게 조정.
- 장르/스타일/사운드/섹션 커스텀/보컬/디테일 팝업의 중립 버튼·카드도 기존보다 살짝만 더 어둡게 맞춰 대비를 보강.
- 선택된 주황 버튼, 기능 동작, 다크모드, Studio Black/분할모드는 유지.

## 817차 — 라이트 하단 생성바 `무작위` 텍스트 갈색 통일
- 기준: 816차.
- 라이트모드 하단 생성바의 `무작위` 텍스트만 816차 보조강조 갈색(`#A2743A`)으로 통일.
- 주사위 아이콘, 버튼 배경/크기/동작, 생성하기/전체초기화, 다크/분할모드는 변경하지 않음.

## 818차 — 라이트 무작위 텍스트 포털 경로 갈색 적용 수정
- 기준: 817차.
- 817차에서 라이트 `무작위` 텍스트가 변하지 않은 원인을 확인: 실제 하단 플로팅 생성바는 `#root` 내부가 아니라 `body` 직속 portal로 렌더링되어 기존 CSS 선택자가 실제 표시 요소에 도달하지 못했다.
- floating portal과 inline action bar를 각각 명시적으로 잡아 `무작위` 텍스트에 갈색 `#A2743A`를 직접 적용했다.
- 주사위 아이콘, 버튼 배경/크기/동작, 생성하기 버튼, 다크/분할모드는 변경하지 않았다.

## 819차 — 라이트 장르~주제 직접입력 확인 체크 갈색 통일
- 기준: 818차.
- 라이트모드 장르/스타일/사운드/분위기/주제 메뉴의 직접입력 상태에서 확인 체크 아이콘만 노란색에서 보조 갈색(`#A2743A`)으로 변경.
- hover는 한 단계 진한 갈색(`#8E6534`)으로만 조정.
- 취소 X, 선택된 주황 버튼, 카드/팝업/레이아웃, 다크모드와 Studio Black/분할모드는 변경하지 않음.

## 821차 — 뮤직노트/라이브러리 디테일 820 과도한 아이보리 덮기 롤백
- 기준: 819차 코드 상태로 복귀. 820차에서 추가한 디테일 전용 `!important` 표면 재도색 블록을 전부 제거했다.
- 820차의 광범위한 `section / rounded-* / bg-* / button` 선택자가 썸네일, 이미지 오버레이, 재생/액션 버튼, 내부 카드 고유색까지 아이보리 계열로 덮던 문제가 원인이었다.
- 뮤직노트/라이브러리 디테일창은 819차의 기존 라이트 디자인과 썸네일/버튼 고유 스타일을 그대로 사용한다.
- 819차까지의 라이트 전체 톤, 갈색 보조강조, 무작위 텍스트, 장르~주제 직접입력 확인 아이콘 변경은 유지한다.
- Firebase/Auth/Firestore/Functions/저장 구조 변경 없음.

## 822차 — 뮤직노트/라이브러리 디테일 광역 덮어쓰기 제거
- 기준: 821차.
- 첨부 영상 재확인 결과, 820차가 아니라 그 이전부터 남아 있던 두 종류의 광역 라이트 규칙이 실제 원인이었다.
  1. `studioLayout.css`의 디테일 전용 규칙이 `rounded-* + bg-*` 모든 하위 요소를 `!important` 아이보리 배경으로 바꾸면서 버튼/미디어 오버레이까지 덮었다.
  2. 816차 `index.css`의 공용 팝업 중립 규칙이 뮤직노트/라이브러리 디테일까지 포함해 모든 `button` 배경을 웜그레이로 강제했다.
- 뮤직노트/라이브러리 디테일을 공용 팝업 중립 규칙에서 완전히 분리했다.
- 디테일 라이트 변환은 이제 외부 shell, header/footer, 실제 `section`, 정보용 div, `bg-white/*` 중립 버튼만 좁게 처리한다.
- 코랄/바이올렛 액션 버튼, 썸네일/커버 이미지, 재생 오버레이, 각 컴포넌트의 고유 배경 효과는 더 이상 공용 아이보리 규칙에 잡히지 않는다.
- Music Note는 기존 코랄, Library는 기존 바이올렛 고유 강조를 유지한다.
- 기능/레이아웃/Firebase/Auth/Firestore/Functions는 변경하지 않았다.

## 823차 — 라이트 디테일창 광원 레이어 깊이 복구
- 기준: 822차.
- 뮤직노트/라이브러리 라이트 디테일창의 썸네일·버튼·고유색 복구 상태는 그대로 유지.
- 라이트 디테일창에서만 패널 전체에 뮤직노트 코랄 / 라이브러리 바이올렛 광원을 아주 약하게 배치.
- 822차의 불투명 아이보리 구조 카드가 광원을 완전히 가리던 문제를 해결하기 위해 실제 구조 섹션/중립 정보칸/입력창만 반투명 웜톤으로 변경.
- 결과적으로 다크/분할처럼 광원이 카드 표면 안쪽으로 은은하게 스며들어 창들이 배경 위에 따로 떠 보이는 느낌을 줄임.
- 이미지/썸네일/미디어 오버레이/코랄·바이올렛 액션 버튼에는 새 색상 덮어쓰기를 추가하지 않음.
- 레이아웃, 기능, Firebase/Auth/Firestore/Functions 변경 없음.

## 824차 — 라이트 상단메뉴 표면 재사용 + 뮤직노트/라이브러리 그림자
- 기준: 823차.
- 라이트모드 Studio 명령창 배경을 상단 네비게이션과 동일한 warm surface(`rgba(248,245,240,0.97)`)로 변경.
- 뮤직노트/라이브러리 검색창도 동일한 상단메뉴 배경색으로 통일.
- 뮤직노트/라이브러리의 탭 메뉴, 색상 필터, 필터 버튼에 외곽선 없이 은은한 그림자를 추가.
- 뮤직노트 곡 카드와 라이브러리 곡 그룹 창에는 메뉴보다 한 단계 깊은 그림자를 적용해 목록 구분감을 보강.
- 선택 코랄/바이올렛 색상, 상세창 광원, 다크모드 및 Studio Black/분할모드는 변경하지 않음.

## 825차 — 라이트 버튼 hover 복구 + 잠금/삭제 상태 배경 강조
- 기준: 824차.
- 라이트모드 Music Note/Library에서 `[class*="bg-white/"]` 공용 `!important` 변환이 `hover:bg-white/*` 클래스까지 같이 잡아 hover 배경 변화를 덮던 문제를 수정했습니다.
- Music Note/Library의 중립 버튼은 마우스를 올리면 한 단계 어두운 웜그레이로 바뀌어 hover가 눈에 보이도록 복구했습니다.
- Music Note 디테일의 잠금 버튼은 잠금 상태에서 아이콘뿐 아니라 버튼 배경 자체도 코랄 틴트로 유지되도록 상태 소유권을 분리했습니다.
- 삭제 버튼은 1회 클릭 확인 상태에서 버튼 배경이 붉은 틴트로 바뀌고, 일반 hover에서도 약한 붉은 배경이 보이도록 수정했습니다.
